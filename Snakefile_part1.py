############# After this initial part of the pipeline, submit {participant}_minCONSCOUNT2*.fasta files to IMGT/High-V Quest.
############# Download resulting IMGT ".txz" files into IMGT_RESULTS folder, extract and proceed to Snakefile_part2.py.

import os
import glob

##### Paths
VIRTUALENV=             '/path/to/py2.7.9_virtualenv'
CHANGEO_PATH=           '/path/to/changeo_0.2.3'
PRESTO_PATH=            '/path/to/presto_v0.4.8'
R_PATH=                 '/path/to/R-3.1.2'
SCRIPTS=                '/path/to/script/folder'
DATA=                   '/path/to/data/folder/'
WDIR=                   DATA+'/Snakemake_wdir'
LOGS=                   WDIR+'/logs'
C_AMPLICONS_REVCOMP=    '/path/to/Allergy_C_amplicons_revcomp.fasta'
R2_PRIMERS=             '/path/to/R2_Primers_Allergy_revcomp.fasta'
R1_PRIMERS=             '/path/to/R1_Primers_Allergy.fasta'
MUSCLE_PATH=            '/path/to/muscle'
...
##### Sample lists
SAMPLES = list(set([os.path.basename(FOLDER) for FOLDER in glob.glob(DATA+'/participant*')]))
...
rule make_chunks_trim_subisotypes:
    input:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass_pair-pass_assemble-pass_missing-pass_reheader_collapse-unique_primers-pass_atleast-2.fasta'
    output:
        WDIR+'/{sample}-fastas-for-IMGT-submission'
    params: name='make_chunks_trim_subisotypes', partition='general', cpus_per_task='4', mem_per_cpu='5300', logfile=LOGS+'/{sample}_make_chunks_trim_subisotypes.log'
    run:
        input_scratch = os.path.basename(input[0])
        output_scratch = os.path.basename(output[0])
        shell("cp {input} $LOCAL_SATA &&\
                cd $LOCAL_SATA &&\
                {R_PATH}/bin/Rscript {SCRIPTS}/make_chunks_trim_subisotypes.R {wildcards.sample} {input_scratch} &&\
...
rule split_conscount:
    input:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass_pair-pass_assemble-pass_missing-pass_reheader_collapse-unique_primers-pass.fastq.gz'
    output:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass_pair-pass_assemble-pass_missing-pass_reheader_collapse-unique_primers-pass_atleast-2.fasta',
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass_pair-pass_assemble-pass_missing-pass_reheader_collapse-unique_primers-pass_under-2.fasta'
...
rule confirm_isotypes:
    input:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass_pair-pass_assemble-pass_missing-pass_reheader_collapse-unique.fastq.gz'
    output:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass_pair-pass_assemble-pass_missing-pass_reheader_collapse-unique_primers-pass.fastq.gz',
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass_pair-pass_assemble-pass_missing-pass_reheader_collapse-unique_primers-fail.fastq.gz'
    ...
    run:
        ...
        shell("cp {input} $LOCAL_SATA &&\
                ...
                source {VIRTUALENV}/bin/activate &&\
                set -o nounset &&\
                python {PRESTO_PATH}/MaskPrimers.py align -s {input_scratch_unzipped} --failed --nproc {params.nproc} --maxerror 0.2 --revpr -p {C_AMPLICONS_REVCOMP} --maxlen 150 --mode cut --log MP_isotype.out &&\
                gzip {output_scratch_unzipped_concat} &&\
                cp {output_scratch_concat} {WDIR}/")

rule collapse_identicals:
    input:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass_pair-pass_assemble-pass_missing-pass_reheader.fastq.gz'
    output:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass_pair-pass_assemble-pass_missing-pass_reheader_collapse-unique.fastq.gz'
...
rule consolidate_conscount:
    input:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass_pair-pass_assemble-pass_missing-pass.fastq.gz'
    output:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass_pair-pass_assemble-pass_missing-pass_reheader.fastq.gz'
...
rule filter_missing:
    input:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass_pair-pass_assemble-pass.fastq.gz'
    output:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass_pair-pass_assemble-pass_missing-pass.fastq.gz'
...
rule assemble_pairs:
    input:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass_pair-pass.fastq.gz',
        WDIR+'/{sample}_R2_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass_pair-pass.fastq.gz'
    output:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass_pair-pass_assemble-pass.fastq.gz',
...
rule pair_seq_2:
    input:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass.fastq.gz',
        WDIR+'/{sample}_R2_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass.fastq.gz'
    output:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass_pair-pass.fastq.gz',
        WDIR+'/{sample}_R2_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass_pair-pass.fastq.gz'
...
rule build_R2_consensus:
    input:
        WDIR+'/{sample}_R2_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass.fastq.gz'
    output:
        WDIR+'/{sample}_R2_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass.fastq.gz',
...
rule build_R1_consensus:
    input:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass.fastq.gz'
    output:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass_consensus-pass.fastq.gz',
...
rule align_sets_R2:
    input:
        WDIR+'/{sample}_R2_quality-pass_primers-pass_allbarcodes-pass_pair-pass.fastq.gz'
    output:
        WDIR+'/{sample}_R2_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass.fastq.gz'
...
rule align_sets_R1:
    input:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass.fastq.gz'
    output:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass_align-pass.fastq.gz'
...
rule pair_seq_1:
    input:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass.fastq.gz',
        WDIR+'/{sample}_R2_quality-pass_primers-pass_allbarcodes-pass.fastq.gz'
    output:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass_pair-pass.fastq.gz',
        WDIR+'/{sample}_R2_quality-pass_primers-pass_allbarcodes-pass_pair-pass.fastq.gz'
...
rule mask_primers_R2_part2:
    input:
        WDIR+'/{sample}_R2_quality-pass_primers-pass.fastq.gz',
        WDIR+'/{sample}_R2_quality-pass_primers-fail.fastq.gz'
    output:
        WDIR+'/{sample}_R2_quality-pass_primers-pass_allbarcodes-pass.fastq.gz',
        WDIR+'/{sample}_R2_quality-pass_primers-fail_primers-pass.fastq.gz',
        WDIR+'/{sample}_R2_quality-pass_primers-fail_primers-fail.fastq.gz',
        WDIR+'/{sample}_MP2_part2.out.gz'
    params: name='mask_primers_R2_part2', partition='long', nproc='12', cpus_per_task='12', mem_per_cpu='5300', logfile=LOGS+'/{sample}_mask_primers_R2_part2.log'
    run:
        input_concat = ' '.join(input)
        input_scratch = [os.path.basename(s) for s in input]
        input_scratch_concat = ' '.join(input_scratch)
        ...
        shell("cp {input_concat} $LOCAL_SATA &&\
                ...
                source {VIRTUALENV}/bin/activate &&\
                set -o nounset &&\
                python {PRESTO_PATH}/MaskPrimers.py score -s {input_scratch_unzipped[1]} --failed --nproc {params.nproc} -p {R2_PRIMERS} --start {UID_LENGTH_R2} --mode cut --barcode --log {output_scratch_unzipped[3]} &&\
                cat {input_scratch_unzipped[0]} {output_scratch_unzipped[1]} > {output_scratch_unzipped[0]} &&\
                gzip {output_scratch_unzipped_concat} &&\
                cp {output_scratch_concat} {WDIR}/")
                
rule mask_primers_R1_part2:
    input:
        WDIR+'/{sample}_R1_quality-pass_primers-pass.fastq.gz',
        WDIR+'/{sample}_R1_quality-pass_primers-fail.fastq.gz'
    output:
        WDIR+'/{sample}_R1_quality-pass_primers-pass_allbarcodes-pass.fastq.gz',
        WDIR+'/{sample}_R1_quality-pass_primers-fail_primers-pass.fastq.gz',
        WDIR+'/{sample}_R1_quality-pass_primers-fail_primers-fail.fastq.gz',
        WDIR+'/{sample}_MP1_part2.out.gz'
    params: name='mask_primers_R1_part2', partition='long', nproc='12', cpus_per_task='12', mem_per_cpu='5300', logfile=LOGS+'/{sample}_mask_primers_R1_part2.log'
    run:
        input_concat = ' '.join(input)
        input_scratch = [os.path.basename(s) for s in input]
        input_scratch_concat = ' '.join(input_scratch)
        ...
        shell("cp {input_concat} $LOCAL_SATA &&\
                ...
                source {VIRTUALENV}/bin/activate &&\
                set -o nounset &&\
                python {PRESTO_PATH}/MaskPrimers.py score -s {input_scratch_unzipped[1]} --failed --nproc {params.nproc} -p {R1_PRIMERS} --start {UID_LENGTH_R1} --mode cut --barcode --log {output_scratch_unzipped[3]} &&\
                cat {input_scratch_unzipped[0]} {output_scratch_unzipped[1]} > {output_scratch_unzipped[0]} &&\
                gzip {output_scratch_unzipped_concat} &&\
                cp {output_scratch_concat} {WDIR}/")
