...
file = args[1]
pat_id = strsplit(file,"_minCONSCOUNT2")[[1]][1]
...
#novel_df = findNovelAlleles(data,germline_ighv,nproc=nproc)
#novel = selectNovel(novel_df)
#if (nrow(novel)>0) {
#for (i in 1:nrow(novel)) {
#  pdf(paste(novel$POLYMORPHISM_CALL[i],"__allele-evidence.pdf",sep=""))
#  plotTigger(data, novel[i,])
#  dev.off()
#}
#geno=inferGenotype(data,find_unmutated=T,germline_db=germline_ighv,novel_df=novel_df)
